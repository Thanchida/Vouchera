...
export function CustomerShell({ title, user, children }: CustomerShellProps) {
  ...
  const navigation = [
    ...
    { label: "My Vouchers", to: "/customer/my-vouchers", end: false },
  ];
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 md:flex">
      <aside className="border-b border-zinc-200 bg-white md:sticky md:top-0 md:h-screen md:w-72 md:border-b-0 md:border-r">
        <div className="px-6 py-5">
          <Link to="/customer" className="block">
            <p className="text-[0.72rem] uppercase tracking-[0.22em] text-zinc-500">Customer</p>
      ...
      <div className="flex-1">
              ...
              {profileOpen ? (
                <div className="absolute right-0 z-20 mt-2 w-64 rounded-2xl border border-zinc-200 bg-white p-2 shadow-lg">
                  <div className="rounded-xl px-3 py-2">
                    ...
                    <p className="text-xs text-zinc-500">Customer account</p>
                  </div>
                  <button
                    type="button"
                    onClick={logout}
        ...
        <main className="mx-auto w-full max-w-7xl px-6 py-6 sm:px-8">{children}</main>
      </div>
    </div>
  );
}