...
export function AdminShell({ title, children }: AdminShellProps) {
  ...
  const [adminUser, setAdminUser] = useState<AuthMe | null>(null);
      ...
      <aside className="border-b border-zinc-200 bg-white md:sticky md:top-0 md:h-screen md:w-72 md:border-b-0 md:border-r">
        <div className="px-6 py-5">
          <Link to="/admin" className="block">
            <p className="text-[0.72rem] uppercase tracking-[0.22em] text-zinc-500">Admin</p>
          ...
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-4 sm:px-8">
            ...
            <div className="relative" ref={profileMenuRef}>
              ...
              >
                ...
                <span className="hidden max-w-44 truncate sm:block">{adminUser?.email ?? "Admin"}</span>
              ...
              {profileOpen ? (
                <div className="absolute right-0 z-20 mt-2 w-64 rounded-2xl border border-zinc-200 bg-white p-2 shadow-lg">
                  <div className="rounded-xl px-3 py-2">
                    <p className="text-sm font-medium text-zinc-900">{adminUser?.email ?? "Admin"}</p>
                    <p className="text-xs text-zinc-500">Admin account</p>
                  </div>
                  <button
                    type="button"
                    onClick={logout}
